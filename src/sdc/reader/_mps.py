...
from wai.spectralio.mps import Reader as SReader

from sdc.api import Reader, Spectrum2D


class MPSReader(Reader, PlaceholderSupporter):
    ...
    def description(self) -> str:
        ...
        :rtype: str
        """
        return "Loads the spectra in MPS format."
    ...
    def _create_argparser(self) -> argparse.ArgumentParser:
        ...
        parser.add_argument("-i", "--input", type=str, help="Path to the ASC file(s) to read; glob syntax is supported; " + placeholder_list(obj=self), required=False, nargs="*")
        parser.add_argument("-I", "--input_list", type=str, help="Path to the text file(s) listing the ASC files to use; " + placeholder_list(obj=self), required=False, nargs="*")
    ...
    def initialize(self):
        ...
        self._inputs = locate_files(self.source, input_lists=self.source_list, fail_if_empty=True, default_glob="*.mps", resume_from=self.resume_from)
